...
class NeoWikiHooks {
	...
	public static function onSkinTemplateNavigationUniversal( SkinTemplate $sktemplate, array &$links ): void {
		...
		if ( !ManageSubjectsAction::isEligibleTitle( $title ) ) {
			return;
		}

		$action = $sktemplate->getRequest()->getRawVal( 'action' );

		$links['views']['neowiki-managesubjects'] = [
			'class' => $action === ManageSubjectsAction::ACTION_NAME ? 'selected' : false,
			'text' => $sktemplate->msg( 'neowiki-managesubjects-tab' )->text(),
			'href' => $title->getLocalURL( [ 'action' => ManageSubjectsAction::ACTION_NAME ] ),
		];
	}
