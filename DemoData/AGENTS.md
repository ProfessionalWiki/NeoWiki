...
| `Module:NeoWikiDemo` `otherSubjectTable` | Builds a wikitable from the current page's Subjects other than its Main Subject (via `mw.neowiki.getSubjects`). Avoids a Cypher query when the data is page-local. Optional `schema=` filter, `sortBy=`/`sortDir=` (default `desc`), `numberColumns=` for thousand-separator formatting. | `{{#invoke:NeoWikiDemo\|otherSubjectTable\|schema=Attendance\|columns=Year, Visitors\|sortBy=Year\|numberColumns=Visitors}}` |
