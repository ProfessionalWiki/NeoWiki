...
[ADR 19](019-graph-database-architecture.md)). In a **wiki farm** (BlueSpice Galaxy), several wikis share **one** graph
so that metadata can be queried across them — this cross-wiki querying is the main reason a graph backend was chosen.
...
not hold across wikis. (Found by @malberts.)

There is also a near-term need: a wiki must be able to **restrict graph queries to its own data**.

The broader Subject Sources work introduces a general `(source, localId)` identity for all Subjects. That is larger and
still being designed; this ADR records the first, demo-driven increment and makes it forward-compatible with that model.
...
- The full namespacing model (subject-id prefixing via Source plugins) remains deferred to the Subject Sources work;
  this stays compatible with it. HW has accepted that the keying may evolve later.

## Alternatives Considered

- **Full `(source, localId)` namespacing now**, applied to subject ids too. More work; requires the whole Subject
  Sources design to settle first. Deferred — HW is fine with the interim, and this increment is forward-compatible.
- **A separate graph per wiki** (no shared graph). Loses cross-wiki querying, which is the farm's reason for a graph
  backend. Rejected.
- **Folding this into [ADR 19](019-graph-database-architecture.md)** rather than a new ADR. Kept separate because
  multi-wiki node identity is a distinct, self-contained decision; ADR 19 and the graph-model reference are updated to
  point at it.
