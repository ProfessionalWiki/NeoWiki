...
- Projections are **pluggable per store**, consistent with [ADR 19](../adr/019-graph-database-architecture.md) (each
  ...
  so loading several into one store means several entries sharing an endpoint (what the development stack does for
  native + EDM).* Running separate stores with different projection sets stays a **deployment choice** — hard
  isolation, independent scaling — not a requirement.
