...
| `GET /neowiki/v0/subject-labels` | Find Subjects of a Schema by name — the label, or for a Main Subject without one, the page title. Other label-less Subjects are absent. Returns each Subject's `id`, `label` and the prefixed `pageTitle` of the page holding it. Query: `schema` (required), `search` (name prefix), `limit`. |
