...
| `POST /neowiki/v0/subjects` | Create a Subject together with a page of its own, in one revision, as that page's main Subject. Body `schema` and `statements`, optional `label`, `pageTitle`, `comment` and `id` (an unused Subject ID to assign; the server mints one when it is omitted). The page is titled by `pageTitle`, or by the label, or by the [template label](schema-format.md#label-template) when there is no label and no page holds that title yet, or by the Subject's ID when none of these titles one. `400` for a malformed `id` or a `pageTitle` that cannot be one; `409` when the `id` is in use or the title is taken, and only the title conflict carries `pageTitle`. |
| `GET /neowiki/v0/subject-labels` | Find Subjects by the `name` the graph gives them ([Graph model](graph-model.md#fixed-properties)); returns `id`/`label` pairs. A Subject without one is absent. Needs a Neo4j store; without one the list is empty. Query: `schema` (only Subjects of that Schema; omit for every Schema), `search` (`name` prefix), `limit`. |
