...
| `name` | string | Subject name, where it has one (see below) |
...
A Subject need not have a label ([ADR 31](../adr/031-optional-subject-labels.md)). One with no label carries its
[template label](schema-format.md#label-template) as `name`; failing that, a Main Subject carries its prefixed page
title, which a page move keeps current, unless that title is the id of a Subject on the page. Any other Subject has
none. A query that must name every Subject can fall back to the Schema label:
