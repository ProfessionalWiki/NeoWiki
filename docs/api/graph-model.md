...
| Property | Neo4j Type | Description |
|----------|------------|-------------|
| `id` | string | Subject ID of a Subject of this wiki, 15 characters starting with `s` (unique) |
...
Only Subjects of this wiki are projected, so `id` is always the bare form; the `sourceKey:localId` form a Subject
from another Source ([ADR 023](../adr/023-subject-sources.md)) carries ([Subject format](subject-format.md#ids)) does not appear here.
...
Relation edges to Subjects from another Source ([ADR 023](../adr/023-subject-sources.md)) are not projected yet;
