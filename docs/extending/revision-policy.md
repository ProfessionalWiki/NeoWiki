...
Choose which revision of a page NeoWiki publishes: to the graph stores, the RDF export, the Subject reads
`GET /neowiki/v0/subject/{subjectId}` and `.../referencingSubjects`, and its own Schema, Layout, Mapping and
configuration reads. Without a policy every page publishes its latest revision.
...
- `publishedRevision()` receives the page's current revision and returns the one to publish, of the same page, or
  ...
  Subject reads answer not-found for it, and leaves its Subjects out of every referencing list. A policy that throws
  counts as returning `null`, with the error logged. It runs on every page save and rebuild and on every read above,
  so keep it cheap.
