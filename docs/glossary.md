...
NeoWiki Statements are not equivalent to Wikibase Statements. The latter has a rank, qualifiers, references, and an ID. For similar modeling, NeoWiki uses Subjects (multiple per page). See [Qualifiers and References](qualifiers-and-references.md) for how to model these.
