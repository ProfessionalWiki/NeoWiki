...
class SubjectSearchHitBuilderTest extends TestCase {
	...
	public function testAPageWithoutContentAndWithoutAMainSubjectIsLeftAlone(): void {
		$this->assertNull( $this->buildHit(
	...
	public function testTheMainSubjectIsPreferredWhenSeveralSubjectsMatch(): void {
		$hit = $this->hitFor(
			$this->pageWith( $this->museumIn( 'Amsterdam' ), $this->otherSubjectIn( 'Amsterdam' ) ),
			false,
			'amsterdam'
		);
	...
	public function testAMatchedLabelIsALineWithoutAPropertyName(): void {
		...
		$this->assertNull( $this->matchOf( $hit )->lines[0]->propertyName );
		$this->assertSame( [ '', 'Rijksmuseum', '' ], $this->matchOf( $hit )->lines[0]->parts );
	}

	public function testAMatchedValueIsALineNamingItsProperty(): void {
		$hit = $this->hitFor( $this->pageWith( $this->museumIn( 'Amsterdam' ) ), true, 'amsterdam' );

		$this->assertSame( 'City', $this->matchOf( $hit )->lines[0]->propertyName );
		$this->assertSame( [ '', 'Amsterdam', '' ], $this->matchOf( $hit )->lines[0]->parts );
	}
	...
	public function testTheMatchedSubjectIsNotNamedAgainWhenThePageTitleNamesIt(): void {
		$hit = $this->hitFor(
			$this->pageWith( $this->subjectWith(
				self::MAIN_ID,
				self::PAGE_NAME,
		...
		$this->assertNull( $this->matchOf( $hit )->subjectName );
	}

	public function testTheMatchedSubjectIsNotNamedAgainWhenItsMatchedLabelShowsTheName(): void {
		$hit = $this->hitFor(
			$this->pageWith( $this->subjectWith(
				self::MAIN_ID,
				'Van Gogh Museum',
	...
	public function testTheMatchedSubjectIsNotNamedAgainWhenTheRowLinksToIt(): void {
		$hit = $this->hitFor( $this->pageWith( $this->museumIn( 'Amsterdam' ) ), false, 'amsterdam' );

		$this->assertNull( $this->matchOf( $hit )->subjectName );
	}

	public function testAGeneratedNameIsNotRepeatedNextToTheSchemaName(): void {
		$hit = $this->hitFor(
			$this->pageWith( $this->subjectWith(
				self::MAIN_ID,
				null,
		...
		$this->assertSame( 'Museum', $this->matchOf( $hit )->schemaName );
		$this->assertNull( $this->matchOf( $hit )->subjectName );
	}
