...
class SubjectIdsByHostingPageTest extends TestCase {
	...
	/**
	 * PageIdentifiersLookup states that ids beyond those requested carry no meaning and are ignored.
	 * Enforcing that here holds whoever implements the lookup to it, and keeps a stray id out of the
	 * grouping, which would otherwise index an id list that does not hold it.
	 */
	public function testIgnoresIdsThatWereNotRequested(): void {
		...
		$this->assertEquals(
			[ 7 => new SubjectIdList( [ $requestedId ] ) ],
