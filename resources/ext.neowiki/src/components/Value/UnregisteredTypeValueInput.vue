<template>
	<CdxField :is-fieldset="false" :disabled="true">
		...
		<div class="ext-neowiki-unregistered-type-value-input">
			<span class="ext-neowiki-unregistered-type-value-input__raw">{{ rawText }}</span>
			<span class="ext-neowiki-unregistered-type-value-input__note">{{ note }}</span>
		</div>
	</CdxField>
</template>
...
const unregisteredTypeValue = computed( () =>
	props.modelValue?.type === ValueType.UnregisteredType ? props.modelValue : undefined );
...
const note = computed( (): string => mw.msg( 'neowiki-property-type-unregistered-input-note', typeName.value ) );

// An unregistered type cannot be interpreted, so the stored value is preserved verbatim
...
.ext-neowiki-unregistered-type-value-input {
	&__raw {
